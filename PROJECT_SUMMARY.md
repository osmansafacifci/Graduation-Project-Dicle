...
> **Status**: All §1–§6.3 result tables complete. §7 standard split conformal prediction is implemented with MAPIE; final interval tables are generated by `3_analysis/conformal_prediction.py`.
...
6. **Geometric alignment fails** — capacity normalization closes the gap
   but does not improve transfer. **The headline result.**
7. **Concept-shift evidence** — KS test on y, per-cell residual
   constant-bias decomposition.
8. **Target-side rescaling works** — k=20 label fix recovers R² from
   −10 to −0.05.
9. **Conformal prediction** — source CP fails under shift; target-domain CP
   restores coverage; residual-mean target adaptation narrows intervals.
10. **Practical recommendation** — quantify shift, take a small target
...
4. **Conformal result selection (SOP §7)**. Run the MAPIE Split-CP script
   for the final reported model set and decide which interval table belongs
   in the manuscript: within-dataset validity, naive cross-dataset failure,
   target-domain calibration, and residual-mean target-adapted calibration.
   Keep the two-parameter linear adapter as sensitivity only.
